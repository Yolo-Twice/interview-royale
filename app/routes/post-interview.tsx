...
const MOCK_REPORT: InterviewReport = {
  ...
  aiSummary:
    "You demonstrated strong fundamentals in React and solid problem-solving skills. Your communication is clear, though you can improve on articulating deep architectural decisions like state management trade-offs.",
  scores: {
    technical: 8.5,
    communication: 8.0,
    problemSolving: 8.2,
  ...
  questions: [
    {
      id: "q1",
      question:
        "How would you optimize a large React application experiencing slow renders?",
      score: 9.0,
      summary:
        "Excellent answer covering useMemo, useCallback, and virtualized lists.",
      detailedFeedback:
        "You correctly identified common bottlenecks and suggested practical solutions. Discussing React DevTools profiling would have made this a perfect answer.",
    },
    {
      id: "q2",
      question:
        "Design a state management solution for a collaborative editing feature.",
      score: 6.5,
      summary: "Missed key concepts around conflict resolution (e.g., CRDTs).",
      detailedFeedback:
        "While Redux/Zustand handles local state well, you didn't adequately address the real-time collaboration aspect. Consider learning about Operational Transformation or CRDTs.",
    },
  ],
  recommendations: [
    {
  ...
  transcript: [
    {
      speaker: "AI",
      line: "Welcome! Let's start with React performance. How would you optimize a large React application experiencing slow renders?",
    },
    {
      speaker: "You",
      line: "I'd start by looking at component re-renders using the React DevTools...",
    },
    {
      speaker: "AI",
      line: "Great. Now let's move to system design. Design a state management solution for a collaborative editing feature.",
    },
    {
      speaker: "You",
      line: "I would use Redux to keep a global store of the document...",
    },
  ],
}
      ...
      try {
        ...
        if (json.success && json.data) {
          ...
          const formattedReport: InterviewReport = {
            ...
            scores: {
              technical:
                data.scores?.technical ?? (data.overallScore || 0) / 10,
              communication:
                data.scores?.communication ?? (data.overallScore || 0) / 10,
              problemSolving:
                data.scores?.problemSolving ?? (data.overallScore || 0) / 10,
              confidence:
                data.scores?.confidence ?? (data.overallScore || 0) / 10,
              systemDesign:
                data.scores?.systemDesign ?? (data.overallScore || 0) / 10,
            },
            strengths: data.strengths || [],
            weaknesses: data.weaknesses || [],
            questions: (data.questionsAnswers || []).map(
              (qa: any, index: number) => ({
                id: `q${index}`,
    ...
    <div className="flex min-h-screen flex-col bg-background pb-12">
      ...
      <main className="mx-auto w-full max-w-5xl flex-1 space-y-8 px-6 py-8 sm:px-8">
        ...
        <section className="flex flex-col gap-6 rounded-xl border bg-card p-6 shadow-sm sm:flex-row sm:items-start sm:p-8">
            ...
            <div>
              <div className="flex items-center gap-2 text-sm text-muted-foreground">
                ...
                <span className="flex items-center gap-1">
                  <Target className="size-3" /> {report.difficulty}
                </span>
                <span>•</span>
                <span className="flex items-center gap-1">
                ...
                <span>•</span>
                <span className="flex items-center gap-1">
                  <Clock className="size-3" /> {report.duration}
                </span>
              </div>
          ...
          <div className="flex min-w-[160px] shrink-0 flex-col items-center justify-center rounded-xl bg-muted/30 p-6">
            ...
            <Badge
              className={`mt-3 border-none ${getScoreColor(report.overallScore)}`}
            >
        ...
        <section>
          ...
          <div className="mt-4 grid gap-4 sm:grid-cols-3 lg:grid-cols-5">
            ...
            <ScoreCard
              icon={Target}
              title="System Design"
              score={report.scores.systemDesign}
            />
          </div>
        </section>
          ...
          <section>
            ...
            <ul className="mt-4 space-y-3">
              {report.strengths.map((strength, i) => (
                <li
                  key={i}
                  className="flex items-start gap-3 rounded-lg bg-emerald-50/50 p-3 text-sm dark:bg-emerald-900/10"
                >
          ...
          <section>
            ...
            <ul className="mt-4 space-y-3">
              {report.weaknesses.map((weakness, i) => (
                <li
                  key={i}
                  className="flex items-start gap-3 rounded-lg bg-amber-50/50 p-3 text-sm dark:bg-amber-900/10"
                >
        ...
        <section className="pt-4">
          ...
          <div className="mt-6">
            <Accordion
              type="multiple"
              className="flex w-full flex-col rounded-none border-none shadow-none"
            >
        ...
        <section className="pt-4">
          ...
          >
            <AccordionItem
              value="transcript"
              className="border-none bg-transparent px-0 shadow-none data-open:bg-transparent"
            >
              <AccordionTrigger className="border-none px-0 py-2 hover:no-underline">
              ...
              <AccordionContent className="px-0 pt-6 pb-2">
                <div className="space-y-6 border-l border-border/50 pl-4">
...
function ReportSkeleton() {
  return (
    <div className="flex min-h-screen flex-col bg-background pb-12">
      ...
      <main className="mx-auto w-full max-w-5xl flex-1 space-y-8 px-6 py-8 sm:px-8">
        <div className="h-48 rounded-2xl border bg-card p-6 shadow-sm">
